...
public class Hovl_Laser : NetMonobehavior, IAttackDetector
{
    ...
    [SerializeField] private DamageFormulaType formulaType;
    public DamageFormulaType Type => formulaType;
    public void Init(object owner, float lifeTime)
    {
    ...
    private void Update()
    {
        ...
        if (Laser != null && UpdateSaver == false)
        {
            ...
            if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, MaxLength))//CHANGE THIS IF YOU WANT TO USE LASERRS IN 2D: if (hit.collider != null)
            {
                ...
                //对敌方造成伤害
                if (hit.collider.GetComponent<ICanInteract>() != null && hit.collider.GetComponent<ICanInteract>().CanBeAttack(hit.collider.GetComponent<CampFlag>()))
                {
                    CharacterDataObj attacker = ownerCBCtrl.GetComponent<CharacterDataController>().data;
                    CharacterDataObj beAttacker = hit.collider.GetComponent<CharacterDataController>().data;

                    float damage = DamageCaculateCollection.CaculateDamage(formulaType, attacker, beAttacker);
                    hit.collider.GetComponent<ICanInteract>().GetDamage(Vector3.zero, damage * Time.deltaTime);
                }
            ...
            if (IsLocal)
            {
                Vector2 mousePos = PlayerInputManager.Instance.action.GamePlay.MousePosition.ReadValue<Vector2>();
                bool checkSucc;
                transform.LookAt(MouseUtility.GetClickedPosition(mousePos, "Ground", out checkSucc));
            }
