...
namespace Micosmo.SensorToolkit
{
    ...
    public class ArcSensor : Sensor, IRayCastingSensor, IPulseRoutine
    {
        ...
        public RayHit GetObstructionRayHit()
        {
            return new RayHit()
            {
                IsObstructing = isObstructed && obstructionRayHit.collider != null,
        ...
        void TestArc()
        {
            ...
            for (var i = 0; i < arcPoints.Count - 1; i++)
            {
                ...
                foreach (var hit in hits)
                {
                    ...
                    if ((1 << hit.collider.gameObject.layer & ObstructedByLayers) != 0 && !signalFilter.ShouldIgnoreObstruction(hit))
                    {
                ...
                {
                    var obspoint = GetObstructionRayHit().Point;
        ...
        protected override void OnDrawGizmosSelected()
        {
            ...
            {
                Gizmos.color = SensorGizmos.GizmoBlockedColor;
            }
            else
            {
                Gizmos.color = SensorGizmos.GizmoColor;
            }
